...
func Test_GetQuotationByQuotationNumber_Input_QT201809_000001_Should_Be_QuotationInfo(t *testing.T) {
	...
	if expectedQuotation.Company != actualQuotation.Company {
		t.Errorf("expect quotation company %v but got it %v", expectedQuotation.Company, actualQuotation.Company)
	}
	if expectedQuotation.Customer != actualQuotation.Customer {
		t.Errorf("expect %v but got it %v", expectedQuotation.Customer, actualQuotation.Customer)
	}
	if expectedQuotation.Payment != actualQuotation.Payment {
		t.Errorf("expect %v but got it %v", expectedQuotation.Payment, actualQuotation.Payment)
	}
	if expectedQuotation.Orders[0] != actualQuotation.Orders[0] {
		t.Errorf("expect %v but got it %v", expectedQuotation.Orders, actualQuotation.Orders)
	}
}
